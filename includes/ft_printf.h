...
{
	va_list arg;
	int len;	// la longueur totale de ce qu'on a print -> on y touche pas,
				// jamais, a l'exeption du return des printf

	int fd;
	char *s_buffer; // si il y a un buffer de retour de sprintf

	char buffer[PRINTF_BUFFER_SIZE]; // le buffer qu'on utilise pour les printf
	size_t size;	// le nombre de char actuellement utile dans le buffer qu'on
					// utilise pour les printf
}				t_data;
...
int ft_printf(char *cmd, ... ); // -> ecrit dans la sortie standard
int ft_sprintf(char *s_buffer, char *cmd, ... ); // Ecrit dans un buffer [s_buffer]
int ft_fprintf(int fd, char *cmd, ... ); // Ecrit dans un fichier [fd]
